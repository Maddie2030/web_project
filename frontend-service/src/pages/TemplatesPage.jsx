...
import DynamicTemplateRenderer from './components/DynamicTemplateRenderer'; // Import the new renderer
...
// Component to render a static template preview (for the main grid)
const StaticTemplateThumbnail = ({ template }) => {
        ...
        <div className="group relative bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-2xl hover:shadow-blue-100/50 transition-all duration-500 transform hover:-translate-y-2 flex-shrink-0 w-80">
            <div className="relative aspect-[395/475] overflow bg-gradient-to-br from-gray-50 to-gray-100 h-96">
                <div className="absolute inset-0 p-4">
                    <StaticTemplateThumbnail template={template} />
                </div>
            </div>
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300">
                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                    ...
                    <button
                        ...
                        className={`p-2 rounded-xl backdrop-blur-sm transition-colors shadow-lg ${isLiked ? 'bg-red-500 text-white' : 'bg-white/90 text-gray-700 hover:text-red-500'}`}
                    >
...
// New component to render a preview of the user's data on a template
const UserTemplateCard = ({ template, navigate, resumeData }) => {
    return (
        <div className="group relative bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-2xl hover:shadow-blue-100/50 transition-all duration-500 transform hover:-translate-y-2 flex-shrink-0 w-80">
            <div className="relative aspect-[395/475] overflow bg-gradient-to-br from-gray-50 to-gray-100 h-96">
                <div className="absolute inset-0 p-4">
                    <DynamicTemplateRenderer template={template} resumeData={resumeData} />
                </div>
            </div>
            <div className="p-6 bg-transparent">
                <h3 className="text-lg font-bold text-gray-900 mb-1">
                    Your resume on this template
                </h3>
                <button
                    onClick={() => navigate(`/text-entry/${template._id || template.id}`)}
                    className="mt-3 bg-blue-600 text-white px-4 py-2 rounded-xl font-medium flex items-center space-x-2 hover:bg-blue-700 transition-colors shadow-lg"
                >
...
const TemplatesPage = () => {
    ...
    const resumeData = useResumeStore((state) => state.resumeData); // Get the user's resume data
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex bg-gray-100 rounded-xl p-1">
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
                                >
                                    <Grid3X3 className="h-5 w-5" />
                                </button>
                                <button
                                    onClick={() => setViewMode('list')}
                                    className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
                                >
                    ...
                    <div className="flex flex-wrap gap-2">
                        {categories.map((category) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${selectedCategory === category ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                            >
                                {category}
                            </button>
                        ))}
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <div className="overflow-x-auto pb-4 scrollbar-hide">
                                <div className="flex space-x-6">
                                    {/* Render the user's resume filled into the first template */}
                                    {filteredTemplates.length > 0 && (
                                        <UserTemplateCard
                                    ...
                                    {/* Render other templates as static previews */}
